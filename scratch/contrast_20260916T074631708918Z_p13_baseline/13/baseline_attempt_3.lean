...
theorem commutative_of_relations {R : Type*} [Ring R] : (∀ a : R, a ^ 3 = a) →
  ...
  have hA : ∀ x : R, 3 * (x - x ^ 2) = 0 := by
    ...
    have h2 : (x - x ^ 2) ^ 3 = 4 * (x - x ^ 2) := by
      calc
        ...
        _ = (x - x ^ 2) * (-2 * (x - x ^ 2)) := by rw [pow_two, h1]
        _ = -2 * (x - x ^ 2) ^ 2 := by noncomm_ring
        _ = -2 * (-2 * (x - x ^ 2)) := by rw [pow_two, h1]
        _ = 4 * (x - x ^ 2) := by noncomm_ring
    have h3 : (x - x ^ 2) ^ 3 = x - x ^ 2 := h (x - x ^ 2)
    have : 4 * (x - x ^ 2) = x - x ^ 2 := by rw [← h2, h3]
    calc
      3 * (x - x ^ 2) = 4 * (x - x ^ 2) - (x - x ^ 2) := by noncomm_ring
      _ = (x - x ^ 2) - (x - x ^ 2) := by rw [this]
      _ = 0 := by noncomm_ring
  ...
  have hE : ∀ x y : R, 2 * (x ^ 2 * y + x * y * x + y * x ^ 2) = 0 := by
    intro x y
    have h_add : x ^ 2 * y + x * y * x + x * y ^ 2 + y * x ^ 2 + y * x * y + y ^ 2 * x = 0 := by
      ...
      calc
        x ^ 2 * y + x * y * x + x * y ^ 2 + y * x ^ 2 + y * x * y + y ^ 2 * x
          = (x + y) ^ 3 - (x + y) := by
            rw [show (x + y) ^ 3 = x ^ 3 + x ^ 2 * y + x * y * x + x * y ^ 2 + y * x ^ 2 + y * x * y + y ^ 2 * x + y ^ 3 by noncomm_ring]
            rw [hx, hy]
            noncomm_ring
        _ = 0 := by rw [h1]; noncomm_ring
    have h_sub : x ^ 2 * y + x * y * x - x * y ^ 2 + y * x ^ 2 - y * x * y - y ^ 2 * x = 0 := by
      have h1 : (x - y) ^ 3 = x - y := h (x - y)
      have hx : x ^ 3 = x := h x
      have hy : y ^ 3 = y := h y
      have hneg : - (x ^ 2 * y + x * y * x - x * y ^ 2 + y * x ^ 2 - y * x * y - y ^ 2 * x) = 0 := by
        calc
          - (x ^ 2 * y + x * y * x - x * y ^ 2 + y * x ^ 2 - y * x * y - y ^ 2 * x)
            = (x - y) ^ 3 - (x - y) := by
              rw [show (x - y) ^ 3 = x ^ 3 - x ^ 2 * y - x * y * x + x * y ^ 2 - y * x ^ 2 + y * x * y + y ^ 2 * x - y ^ 3 by noncomm_ring]
  ...
  have hF : ∀ a b : R, 2 * (a * b - b * a) = 0 := by
    ...
    have h4' : 2 * (a * b + a ^ 2 * b * a + a * b * a ^ 2) = 0 := by
      have ha : a ^ 3 = a := h a
      calc
        2 * (a * b + a ^ 2 * b * a + a * b * a ^ 2)
          = 2 * (a ^ 2 * (a * b) + a * (a * b) * a + (a * b) * a ^ 2) := by
    ...
    have h5' : 2 * (a ^ 2 * b * a + a * b * a ^ 2 + b * a) = 0 := by
      have ha : a ^ 3 = a := h a
      calc
        2 * (a ^ 2 * b * a + a * b * a ^ 2 + b * a)
          = 2 * (a ^ 2 * (b * a) + a * (b * a) * a + (b * a) * a ^ 2) := by
